    ...
    return (
        <div className="w-full overflow-x-hidden h-full text-gray-700">
            <AnimatePresence>
                ...
                {(suratDibaca.nomor < 0) && <motion.div key={"daftarSurat"} className=" w-96 sm:80 absolute top-0 left-1/2 -translate-x-1/2" transition={{ type: "tween", ease: "easeOut" }} initial={variant.initial} exit={variant.exit} animate={variant.animate}><DaftarSurat bacaSuratCb={handleBacaSurat} /></motion.div>}
                {(suratDibaca.nomor > 0) && <motion.div key={"pembacaSurat"} transition={{ type: "tween", ease: "easeOut" }} initial={variant.initial} animate={variant.animate} exit={variant.exit} className="w-96 sm:w-80 absolute left-1/2  ">
